...
if ( ! function_exists( 'noo_widget_title_display_callback' ) ) :

	function noo_widget_title_display_callback($instance, $widget,$args){
		if (isset($instance['title'])){
			$title = $instance['title'];
			if(!empty($title))
				$instance['title'] = noo_nth_word($title,'first',false);
		}
		return $instance;
	}

	add_filter('widget_display_callback','noo_widget_title_display_callback',10,3);

endif;

if ( ! function_exists( 'noo_widget_title_filter' ) ) :
	function noo_widget_title_filter($title){
		return wp_specialchars_decode($title);
	}
	add_filter( 'widget_title', 'noo_widget_title_filter',100 );
endif;