	...
	class NooMailChimp {
		...
		public function ajax_mc_subscribe() {
			if ( ! check_ajax_referer( 'noo-subscribe', 'nonce', false ) ) {
				$this->_ajax_exit( esc_html__( 'Your session is expired. Please reload and retry.', 'noo' ) );
			}
			...
			if(!is_email($email)) {
				$this->_ajax_exit( wp_kses( __('Your email address is invalid. <span class="reloadpage">Click Reload</span> and enter a valid email address.', 'noo' ), noo_allowed_html() ), esc_html__('Invalid Email', 'noo' ) );
			}

			$list_id = strip_tags($_POST['mc_list_id']);

			if(empty($list_id)) {
				$this->_ajax_exit(esc_html__( 'There\'s unknown problem. Please reload and retry.', 'noo' ));
			}
			
			// send this email to campaign_monitor
			if( $this->_subscribe_email($email, $list_id) ) {
				setcookie('noo_subscribed', 1, time()*20, '/');
				$this->_ajax_exit( esc_html__( 'Thank you for your subscription.', 'noo' ), true );
			} else {
				$this->_ajax_exit($list_id. ' ' .$email. ' ' . esc_html__( 'There\'s unknown problem. Please reload and retry.', 'noo' ));
			}
		}
