...
if (!function_exists('noo_get_featured_video')):
	function noo_get_featured_video($prefix = '_noo_wp_post', $post_id = null,$lightbox=false,  $is_shortcode = false) {
		...
		if ( $embed != '' ) {
			...
			<div id="noo-video-container<?php echo esc_attr($post_id); ?>" class="noo-video-container <?php echo esc_html($ratio_class); ?>" <?php if($lightbox_preview):?> style="display: none;"<?php endif;?>>
...
if (!function_exists('noo_get_featured_trainer')):
	function noo_get_featured_trainer($post_id = null,$image_size = '', $post_format = '',$lightbox = false, $is_shortcode = false) {
		$post_id = (null === $post_id) ? get_the_id() : $post_id;
...
if (!function_exists('noo_featured_trainer')):
	function noo_featured_trainer($post_id = null, $post_format = '',$lightbox=false,$is_shortcode = false) {
		echo noo_get_featured_trainer( $post_id, $post_format, $lightbox, $is_shortcode);
	}
endif;
	...
	function noo_get_action_portfolio($post_id = null, $post_format = '',$lightbox = false, $is_shortcode = false) {
		...
		switch ($post_format) {
			case 'image':
				...
				return implode($html, "\n");
			case 'link':
				$html[] = '<a class="noo-share" href="'.$link.'" >';
