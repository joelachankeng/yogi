...
if ( defined( 'WPB_VC_VERSION' ) ) :
	...
	if ( ! function_exists( 'noo_vc_custom_param_event_categories' ) ) :

		function noo_vc_custom_param_event_categories( $settings, $value ) {
			$dependency = ( function_exists( 'vc_generate_dependencies_attributes' ) ) ? vc_generate_dependencies_attributes( $settings ) : '';
			$categories = get_terms( 'event_category', array( 'orderby' => 'NAME', 'order' => 'ASC' ) );
			$class = 'wpb-input wpb-select ' . $settings['param_name'] . ' ' . $settings['type'] . '_field';
			$selected_values = explode( ',', $value );
			$html = array( '<div class="noo_vc_custom_param event_categories">' );
			$html[] = '  <input type="hidden" name="' . $settings['param_name'] . '" value="' . $value .
				 '" class="wpb_vc_param_value" />';
			$html[] = '  <select name="' . $settings['param_name'] . '-select" multiple="true" class="' . $class . '" ' .
				 $dependency . '>';
		...
		vc_add_shortcode_param( 'event_categories', 'noo_vc_custom_param_event_categories' );
	
	
	endif;
	
	if ( ! function_exists( 'noo_vc_custom_param_event_locations' ) ) :

		function noo_vc_custom_param_event_locations( $settings, $value ) {
			$dependency = ( function_exists( 'vc_generate_dependencies_attributes' ) ) ? vc_generate_dependencies_attributes( $settings ) : '';
			$locations = get_terms( 'event_location', array( 'orderby' => 'NAME', 'order' => 'ASC' ) );
			$class = 'wpb-input wpb-select ' . $settings['param_name'] . ' ' . $settings['type'] . '_field';
			$selected_values = explode( ',', $value );
			$html = array( '<div class="noo_vc_custom_param event_locations">' );
			$html[] = '  <input type="hidden" name="' . $settings['param_name'] . '" value="' . $value .
				 '" class="wpb_vc_param_value" />';
			$html[] = '  <select name="' . $settings['param_name'] . '-select" multiple="true" class="' . $class . '" ' .
				 $dependency . '>';
		...
		vc_add_shortcode_param( 'event_locations', 'noo_vc_custom_param_event_locations' );
	...
	if ( ! function_exists( 'noo_vc_custom_param_class_category' ) ) :

		function noo_vc_custom_param_class_category( $settings, $value ) {
			$dependency = ( function_exists( 'vc_generate_dependencies_attributes' ) ) ? vc_generate_dependencies_attributes( $settings ) : '';
			$categories = get_terms( 'class_category', array( 'orderby' => 'NAME', 'order' => 'ASC' ) );
			$class = 'wpb-input wpb-select ' . $settings['param_name'] . ' ' . $settings['type'] . '_field';
			$selected_values = explode( ',', $value );
		...
		vc_add_shortcode_param( 'class_category', 'noo_vc_custom_param_class_category' );
	
	
	endif;

	if ( ! function_exists( 'noo_vc_custom_param_trainer_multi' ) ) :

		function noo_vc_custom_param_trainer_multi( $settings, $value ) {

			$trainers = array();
			$trainers[__('All trainer','noo')] = 0;
			...
			foreach ( $trainers as $key => $trainer ) {
				$html[] = '    <option value="' . $trainer . '" ' .
					 ( in_array( $trainer, $selected_values ) ? 'selected="true"' : '' ) . '>';
				$html[] = '      ' . $key;
				$html[] = '    </option>';
			}
		...
		vc_add_shortcode_param( 'trainer_multi', 'noo_vc_custom_param_trainer_multi' );
...
if ( defined( 'WPB_VC_VERSION' ) ) :
	if ( ! function_exists( 'noo_vc_admin_enqueue_assets' ) ) :
		...
		function noo_vc_admin_enqueue_assets( $hook ) {
			...
			wp_register_style( 
				'noo-vc-admin-css', 
				NOO_FRAMEWORK_ADMIN_URI . '/assets/css/noo-vc-admin.css', 
...
if ( defined( 'WPB_VC_VERSION' ) ) :
	...
	if ( ! function_exists( 'noo_vc_remove_unused_elements' ) ) :
		...
		function noo_vc_remove_unused_elements() {
			...
			// vc_remove_element( 'ntt_trainer' );
			// vc_remove_element( 'ntt_event' );
			// vc_remove_element( 'ntt_class' );
		}
		
		add_action( 'admin_init', 'noo_vc_remove_unused_elements', 10 );
...
if ( defined( 'WPB_VC_VERSION' ) ) :
	if ( ! function_exists( 'noo_vc_base_element' ) ) :
		...
		add_action( 'admin_init', 'noo_vc_base_element', 11 );
	...
	// NOO Yogi
	if ( ! function_exists( 'noo_vc_yogi' ) ) {

		function noo_vc_yogi() {
			//
			// Variables.
			//
			$category_name = esc_html__( 'By Yogi', 'noo' );
			$category_plugin_name = esc_html__( 'By NooTimetable', 'noo' );
			...
			// Extend from plugin Timetable
			if ( class_exists('Noo__Timetable__Main') ) {

				vc_map( array(
				    'name'          =>  esc_html__( 'Noo Trainer', 'noo' ),
				    'base'          =>  'ntt_trainer',
				    'description'   =>  '',
				    'icon'          =>  '',
				    ...
				    'params'        =>  array(
				        ...
				        array(
				            'param_name'  => 'title',
				            'heading'     => esc_html__( 'Title (optional)', 'noo' ),
				            'description' => '',
				            'type'        => 'textfield',
				            'value'       => ''
				        ),
				        ...
				        array(
				            ...
				            'heading'     => esc_html__( 'Sub Title (optional)', 'noo' ),
				            'description' => '',
				            'type'        => 'textfield',
				            'value'       => ''
				        ),
				        ...
				        array(
				            ...
				            'heading'     => esc_html__( 'Layout Style', 'noo' ), 
				            'type'        => 'dropdown', 
				            'admin_label' => true,
				            'value'       => array( 
				                esc_html__( 'Grid', 'noo' )   => 'grid',
				                esc_html__( 'List', 'noo' )   => 'list'
				            ), 
				        ),

				        array( 
				            'param_name' => 'columns', 
				            'heading'    => esc_html__( 'Columns', 'noo' ), 
				            'type'       => 'ui_slider',
				            'admin_label' => true,
				            'value'      => '4', 
				            'data_min'   => '1', 
				        ...
				        array( 
				            'param_name'  => 'cat', 
				            'heading'     => esc_html__( 'Trainer Categories', 'noo' ), 
				            'description' => '', 
				        ...
				        array( 
				            'param_name' => 'limit', 
				            'heading'    => esc_html__( 'Max Number of Trainers', 'noo' ), 
				            'type'       => 'ui_slider', 
				            'admin_label' => true,
				            'value'      => '4', 
				            'data_min'   => '1', 
			...
			vc_map( 
				array( 
					...
					'params' => array( 
						...
						array( 
							"type" => "dropdown", 
							"heading" => __( "Products Ordering", 'noo' ), 
							"param_name" => "orderby", 
							'class' => 'dhwc-woo-product-page-dropdown', 
							"value" => array( 
								__( 'Publish Date', 'noo' ) => 'date', 
								__( 'Modified Date', 'noo' ) => 'modified', 
								__( 'Random', 'noo' ) => 'rand', 
								__( 'Alphabetic', 'noo' ) => 'title', 
								__( 'Popularity', 'noo' ) => 'popularity', 
								__( 'Rate', 'noo' ) => 'rating', 
								__( 'Price', 'noo' ) => 'price' ) ), 
						array( 
							"type" => "dropdown", 
							"class" => "", 
							"heading" => __( "Sorting", 'noo' ), 
							"param_name" => "order", 
							"value" => array( 
								__( 'Ascending', 'noo' ) => 'ASC', 
								__( 'Descending', 'noo' ) => 'DESC' ) ), 
						array( 
							'param_name' => $param_visibility_name, 
							'heading' => $param_visibility_heading, 
							'description' => $param_visibility_description, 
			...
			if ( class_exists('Noo__Timetable__Main') ) {
				    ...
				    'params'        =>  array(
				        ...
				        array( 
							...
							'value'       => array( 
								...
								__( 'Info Overlay', 'noo' ) => 'info_overlay'
							),
						),
				        array( 
				            'param_name'  => 'show_info', 
				            'heading'     => esc_html__( 'Show Info', 'noo' ), 
				            'type'        => 'dropdown', 
				            'admin_label' => true,
				            'value'       => array( 
								__( 'Show Date & Time', 'noo' )  => 'all', 
								esc_html__( 'Only Date', 'noo' ) => 'date',
								esc_html__( 'Only Time', 'noo' ) => 'time',
								__( 'Hide Date & Time', 'noo' )  => 'null'
				            ),
				        ),
				        array( 
							'param_name'  => 'columns', 
				        ...
				        array( 
				            'param_name'  => 'cat', 
				            'heading'     => esc_html__( 'Class Categories', 'noo' ), 
				            'description' => '', 
				        ...
				        array( 
				            ...
				            'value'       => array( 
				                esc_html__( 'Default', 'noo' )                 => 'default',
				                esc_html__( 'Open Date', 'noo' )               => 'open_date',
				                esc_html__( 'Recent First', 'noo' )            => 'latest',
				                esc_html__( 'Older First', 'noo' )             => 'oldest',
				                esc_html__( 'Title Alphabet', 'noo' )          => 'alphabet',
				                esc_html__( 'Title Reversed Alphabet', 'noo' ) => 'ralphabet'
				        ...
				        array( 
							'param_name'  => 'limit', 
							'heading'     => esc_html__( 'Max Number of Classes', 'noo' ), 
							'type'        => 'ui_slider',
							'admin_label' => true,
							'value'       => '4', 
							'data_min'    => '1', 
				...
				// Coming Class
				vc_map( array(
				    'name'          =>  esc_html__( 'Noo Class Coming', 'noo' ),
			...
			if ( class_exists('Noo__Timetable__Main') ) {
				...
				vc_map( array(
				    ...
				    'params'        =>  array(
				        ...
				        array( 
				            'param_name'  => 'cat', 
				            'heading'     => esc_html__( 'Event Categories', 'noo' ), 
				            'description' => '', 
				        ...
				        array( 
				            ...
				            'value'       => array( 
				                esc_html__( 'Default', 'noo' )                 => 'default',
				                esc_html__( 'Start Date', 'noo' )              => 'start_date',
				                esc_html__( 'Recent First', 'noo' )            => 'latest',
				                esc_html__( 'Older First', 'noo' )             => 'oldest',
				                esc_html__( 'Title Alphabet', 'noo' )          => 'alphabet',
				                esc_html__( 'Title Reversed Alphabet', 'noo' ) => 'ralphabet'
				        ...
				        array( 
				            'param_name' => 'limit', 
				            'admin_label' => true,
				            'heading'    => esc_html__( 'Max Number of Events', 'noo' ), 
				            'type'       => 'ui_slider',
				            'value'      => '4', 
				            'data_min'   => '1', 
				            'data_max'   => '50'
						...
						array( 
							'param_name'  => 'hide_past_events', 
							'heading'     => __( 'Hide Past Events', 'noo' ), 
							'type'        => 'checkbox',
							'value'       => array( '' => 'true' )
						),
						array( 
							'param_name'  => 'hide_events_time', 
							'heading'     => __( 'Hide Events Time', 'noo' ), 
							'type'        => 'checkbox',
							'value'       => array( '' => 'true' )
						),
						array( 
							'param_name'  => 'hide_events_date', 
							'heading'     => __( 'Hide Events Date', 'noo' ), 
							'type'        => 'checkbox',
							'value'       => array( '' => 'true' )
						),
						array( 
							'param_name'  => 'hide_events_location', 
							'heading'     => __( 'Hide Events Location', 'noo' ), 
							'type'        => 'checkbox',
							'value'       => array( '' => 'true' )
						),
						array( 
		...
		add_action( 'admin_init', 'noo_vc_yogi', 11 );
		
		if ( class_exists( 'WPBakeryShortCodesContainer' ) ) {

			class WPBakeryShortCode_Event_Slider extends WPBakeryShortCodesContainer {
			}

			class WPBakeryShortCode_Cause_Slider extends WPBakeryShortCodesContainer {
			}
		}
	}
	//
	...
	if ( ! function_exists( 'noo_vc_typography' ) ) :
		...
		add_action( 'admin_init', 'noo_vc_typography', 11 );
	...
	if ( ! function_exists( 'noo_vc_content' ) ) :
		...
		function noo_vc_content() {
			...
			// [vc_accordion]
			// ============================
			vc_map_update( 'vc_accordion', array( 'category' => $category_content, 'weight' => 790 ) );
			...
			vc_add_param( 
				...
				array( 
					'param_name'  => 'active_tab', 
					'heading'     => __( 'Active Tab', 'noo' ), 
					'description' => __( 'The tab number to be active on load, default is 1. Enter -1 to collapse all tabs.', 
					'noo' ), 
					'type'        => 'textfield', 
					'holder'      => $param_holder 
				)
			...
			array(
				...
				'params'          => array(
					...
					array( 
						'param_name' => 'purchase_btn_type', 
						'heading'    => __( 'Style', 'noo' ), 
						'type'       => 'dropdown', 
						'holder'     => $param_holder, 
						'value'      => array( 
							__( 'Default', 'noo' )  => 'default', 
							__( 'Healcode', 'noo' ) => 'healcode' 
						) 
					),
					array(
						'param_name' => 'healcode_btn_code',
						'heading'    => __( 'Healcode  Purchase button code', 'noo' ),
						'type'       => 'textarea_safe',
						'holder'     => $param_holder,
						'dependency' => array( 'element' => 'purchase_btn_type', 'value' => array( 'healcode' ) ),
					),
					array(
						...
						'type'       => 'textfield',
						'holder'     => $param_holder,
						'dependency' => array( 'element' => 'purchase_btn_type', 'value' => array( 'default' ) ),
		...
		add_action( 'admin_init', 'noo_vc_content', 11 );
	...
	if ( ! function_exists( 'noo_vc_wp_content' ) ) :
		...
		add_action( 'admin_init', 'noo_vc_wp_content', 11 );
	...
	if ( ! function_exists( 'noo_vc_media' ) ) :
				...
				array( 
					...
					'params'          => array( 
						array( 
							'param_name'  => 'type', 
							'heading'     => __( 'Type', 'noo' ), 
							'description' => __( 'Choose the type of this slide: Image, Video or HTML Content', 
							'noo' ), 
							'type'        => 'dropdown', 
							'holder'      => $param_holder, 
							'value'       => array( 
		...
		add_action( 'admin_init', 'noo_vc_media', 11 );
	...
	if ( ! function_exists( 'noo_vc_other' ) ) :
		...
		add_action( 'admin_init', 'noo_vc_other', 11 );
